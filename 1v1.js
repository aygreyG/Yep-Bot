...
class Game {
  ...
  calcWinnerString(authornum, opponentnum) {
    if (Math.abs(authornum - this.rand) < Math.abs(opponentnum - this.rand)) {
      ...
      Math.abs(authornum - this.rand) > Math.abs(opponentnum - this.rand)
    ) {
      this.currency.add(this.opponentid, this.bet);
      this.currency.add(this.authorid, -this.bet);
      ...
      return (
        "It is a draw, noone wins! Because the number was: " + this.rand + "."
      );
    }
  }
}
...
const oneVOne = async (channel, currency, authorid, opponentid) => {
  if (
    currency.getBalance(authorid) <= 0 ||
    currency.getBalance(opponentid) <= 0
  ) {
    channel.send({
      embeds: [
  ...
  const maxbet =
    currency.getBalance(opponentid) >= currency.getBalance(authorid)
      ? currency.getBalance(authorid)
      : currency.getBalance(opponentid);
  ...
  const betMessage = await channel.send({
    embeds: [
      new Discord.MessageEmbed()
        .setColor("BLURPLE")
        .setDescription(
          `<@${authorid}> give me a bet with '${prefix}bet1v1 <amount>'. The maximum amount is: ${maxbet}. (You have 10 seconds)`
        ),
    ],
  });
  ...
  const filter = (m) => {
    return (
      ...
      m.content.startsWith(prefix + "bet1v1") &&
      parseInt(m.content.slice(7).trim()) <= maxbet
    );
  };
...
const gameL = (game, channel) => {
  ...
  const msgFilter = (msg) => {
    return (
      ((msg.author.id == game.authorid && authornum == -1) ||
  ...
  channel.send({
    embeds: [
      new Discord.MessageEmbed()
        .setColor("BROWN")
        .setDescription(
          `<@${game.authorid}>, <@${game.opponentid}> give me a number between 0-100, if you guess closer you win! (You have 10 seconds)`
        ),
    ],
  });
    ...
    if (reason === "time") {
      channel.send({
        embeds: [
          new Discord.MessageEmbed()
            .setColor("RED")
            .setDescription("Someone did not give a number... 😡"),
        ],
      });
    }
  });
...
const isBetValid = async (channel, opponentid, authorid, bet, currency) => {
  const betaccept = await channel.send({
    embeds: [
      new Discord.MessageEmbed()
        .setColor("BLUE")
        .setDescription(
          `<@${opponentid}> react to this message if the bet(${bet}) is good for you! (You have 5 seconds)`
        ),
    ],
  });
  ...
  const reactionC = betaccept.createReactionCollector({
